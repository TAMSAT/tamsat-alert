...
def richa_num(P_val, p_val, T_val, u_val, q1_val, qsat, h, fa_val, gs, e_psi, LAI, dt_val):
    ...
    #---------------------------------------------------#
    if LAI == 0.0:
        zo = 3.0 * 10**-4 # surface roughness length (m)
    else:
        zo = h / 10.0
...
def qsat_ra_rc(P_val, p_val, T_val, dt_val):
    ...
    log_es = ((10.79574 * (1 - (273.16 / T_val))) \
              - (5.028 * (math.log10( T_val / 273.16))) \
              + (1.50475 * (10**-4) * (1-(10**(-8.2969*((T_val / 273.16)-1))))) \
              + (0.42873 * (10**-3) * ((10**(-4.76955*(1-(273.16 / T_val))))-1)) \
              + (0.78614 + 2.0))
...
def calc_ch(LAI,h,Rib,u_val):
    ...
    chn = (vkman**2) * (((math.log((z1 + zo)/zo)) *\
                    (math.log((z1 + zo)/ zoh)))**-1)
    ...
    if Rib >= 0.0:
        fh = (1.0 + (10.0*(Rib/Pr)))**-1
##        fh = (1.0 + (10.0*Rib))**-1
    else:
        fh = 1.0 - (10.0 * Rib * ((1 + (10*chn* (np.sqrt(-Rib))/fz))**-1))
##        fh = 1.0 - (10.0 * Rib / (1 + bh * (np.sqrt(-Rib))))

    ch = fh * chn

    ra = (fh * chn * (abs(u_val)))**-1
...
def tf_runoff_inf(P_val, LAI , model_t_step, er, Ks, I_v, Ec):
    ...
    if P_val == 0:
        ...
        if C < Cm:
            tf = (P_val * (1 -(C / Cm)) * math.exp(((- er * Cm)/(P_val * model_t_step)))) + \
                 (P_val * (C / Cm))
            Tf = np.append(Tf, tf)
#            print tf * 86400
            if (K * model_t_step) <= C:
                y = ((P_val * (C / Cm)) * math.exp(((- er * K* Cm)/(P_val * C)))) + \
                   (P_val * (1 -(C / Cm)) * math.exp(((- er * Cm)/(P_val * model_t_step))))
                Y = np.append(Y, y)
...
def cal_av_beta_fc(theta_s, theta_c, theta_w, Su, rk):
    ...
    for z in range(0,len(Su)):
        for t in range(0,len(Su[0])):
            if theta[z,t] > theta_fc:
                beta = 1
                beta_vals[z,t] = beta
                pfc_vals[z,t] = 1 # percentage field capacity
    ...
    for z in range(0,len(Su)):
        for t in range(0,len(Su[0])):
            if theta[z,t] > theta_fc:
                pfc_vals[z,t] = 1 # above FC 100%
            elif theta[z,t] <= theta_w:
                pfc_vals[z,t] = 0 # below PWP 0%
            else:
...
def calc_ek(rk, theta_c, theta_w, beta, LAI, gl, theta):
    ...
    g_soil = 0.01 * ((thetaval / theta_c)**2) # bare soil evaporation

    fr = 1 - (math.exp( - LAI / 2.0)) # radiative fraction

    fpar = (1 - (math.exp(- 0.5 * LAI))) / 0.5 # factor influencing canopy conductance

    gc = gl * fpar # canopy conductance (m/s)

    gs = gc + ((1 - fr) * g_soil) # surface conductance (m/s)
...
def evapo_flux(fa_val, ra, q1_val, qsat, beta, C, ch, u_val, gs, model_t_step):
    ...
    if (Ec * model_t_step) > C:
        Es = e_psi_s * (1 - ((fa_val * C) / (Ec * model_t_step))) * Eo
#        print '>C: ',Es
    else:
        Es = (1 - fa_val)* e_psi_s * Eo
